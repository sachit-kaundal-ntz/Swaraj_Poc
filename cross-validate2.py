...
Cross-validates extracted JSON data against technical drawings using Gemini 2.5 Flash
Enhanced focus on dimension ID validation with improved gear component classification
Added large image handling and compression capabilities
"""
...
# Increase PIL's image size limit for large technical drawings
Image.MAX_IMAGE_PIXELS = 300000000  # Allow up to 300MP images
...
"""
    ...
    def load_and_process_image(self, image_path: str, max_size: Tuple[int, int] = (4096, 4096)) -> Image.Image:
        """Load and process image file with size optimization for Gemini"""
        try:
            print(f"Loading image: {image_path}")
            
            # Load the image with increased size limits
            with Image.open(image_path) as img:
                print(f"Original image size: {img.size} ({img.size[0] * img.size[1]} pixels)")
                ...
                if img.size[0] > max_size[0] or img.size[1] > max_size[1]:
                    print(f"Resizing image to fit within {max_size}")
    ...
    def compress_image_for_api(self, image: Image.Image, max_file_size_mb: float = 20) -> Image.Image:
        """Compress image for API upload while maintaining quality"""
        try:
            ...
            while quality > 50:
                ...
                print(f"Compressed image size at quality {quality}: {size_mb:.2f} MB")
            ...
            if size_mb > max_file_size_mb:
                print("Further reducing image dimensions...")
                current_size = image.size
                reduction_factor = 0.8
                
                while size_mb > max_file_size_mb and reduction_factor > 0.3:
                    new_size = (int(current_size[0] * reduction_factor), 
                               int(current_size[1] * reduction_factor))
                    ...
                    print(f"Resized to {new_size}, size: {size_mb:.2f} MB")
    ...
    def load_image(self, image_path: str) -> Image.Image:
        """Load image with enhanced processing for large technical drawings"""
        # First load and process the image
        processed_image = self.load_and_process_image(image_path)
        
        # Compress for API if needed
        final_image = self.compress_image_for_api(processed_image)
        
        print(f"Final processed image size: {final_image.size}")
        return final_image
    ...
    def _parse_dimension_id_format(self, response_text: str, debug: bool = False) -> List[ValidationResult]:
        ...
        # Look for dimension ID patterns like "dim_outer_dia", "dim_hub_dia", etc.
        dim_id_pattern = r'(?:VALIDATION_ITEM:|ITEM:|ID:)\s*(dim_[a-zA-Z_]+|[a-zA-Z_]*[dD]ia[a-zA-Z_]*|[a-zA-Z_]*[bB]ore[a-zA-Z_]*|[a-zA-Z_]*[wW]idth[a-zA-Z_]*)'
        matches = re.finditer(dim_id_pattern, response_text, re.IGNORECASE)
...
VALIDATION_ITEM: {table_id}_entry
CATEGORY: tables
JSON_VALUE: [value from JSON with unit]
IMAGE_VALUE: [value from drawing table]
STATUS: PASS/FAIL/WARNING/MISSING
...
Check each parameter-value pair precisely against the drawing tables.
"""
...
"""
    ...
    def run_comprehensive_validation(self, json_path: str, image_path: str, debug: bool = False) -> Dict[str, Any]:
        ...
        print(f"Loading and processing image from: {image_path}")
        image = self.load_image(image_path)
        ...
        return {
            ...
            'image_processing_info': {
                'original_size_handling': 'Large image support enabled',
                'compression_applied': 'Auto-compression for API compatibility',
                'max_pixels_allowed': Image.MAX_IMAGE_PIXELS
            },
            'summary': summary,
            'detailed_results': [
                {
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Enhanced cross-validation of JSON extracted data against technical drawings with focus on dimension ID validation and large image support')
    parser.add_argument('json_file', help='Path to JSON file with extracted data')
    parser.add_argument('image_file', help='Path to technical drawing image')
    parser.add_argument('--api-key', required=True, help='Google Gemini API key')
    parser.add_argument('--model', default='gemini-2.5-flash', help='Gemini model to use')
    parser.add_argument('--output', help='Output file for validation results (default: auto-generated)')
    parser.add_argument('--debug', action='store_true', help='Enable debug output')
    parser.add_argument('--max-size', nargs=2, type=int, default=[4096, 4096], 
                        help='Maximum image dimensions for processing (width height)')
    parser.add_argument('--max-file-size', type=float, default=20.0, 
                        help='Maximum file size in MB for API upload')
    ...
    try:
        print(f"Enhanced Technical Drawing Validator v2.0")
        print(f"Large image support enabled (max pixels: {Image.MAX_IMAGE_PIXELS:,})")
        print(f"Max processing size: {args.max_size[0]}x{args.max_size[1]}")
        print(f"Max API upload size: {args.max_file_size} MB")
        print("-" * 70)
        
        # Initialize validator
        validator = TechnicalDrawingValidator(args.api_key, args.model)
        
        # Update image processing parameters if provided
        if hasattr(validator, 'max_processing_size'):
            validator.max_processing_size = tuple(args.max_size)
        if hasattr(validator, 'max_api_file_size'):
            validator.max_api_file_size = args.max_file_size
        ...
        if 'error' in summary:
            ...
            # Print image processing info
            if 'image_processing_info' in results:
                img_info = results['image_processing_info']
                print(f"\nIMAGE PROCESSING:")
                print(f"  Status: {img_info['original_size_handling']}")
                print(f"  Compression: {img_info['compression_applied']}")
                print(f"  Max pixels allowed: {img_info['max_pixels_allowed']:,}")
        ...
        print(f"\nEnhanced validation completed. Results saved to: {args.output}")
        print(f"Large image processing: Successfully handled")
