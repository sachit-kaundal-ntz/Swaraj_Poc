...
"""
drwaingRouter.py
FastAPI router for the Gear Drawing Cost Estimation pipeline.
Location: app/routers/drwaingRouter.py

Existing endpoints (preserved exactly):
  POST /upload                    Upload a single drawing → background processing
  GET  /status/{task_id}          Processing status
  GET  /result/{task_id}          Gemini extraction result
  GET  /download/{task_id}/json   Download JSON output
  GET  /list                      List all processed drawings (filterable)
  DELETE /delete/{task_id}        Delete drawing + files
  GET  /health                    Service health check
  POST /batch-upload              Upload up to 10 drawings at once
  GET  /stats                     Processing statistics
  POST /reprocess/{task_id}       Re-trigger processing on an existing task

New pipeline endpoints (added):
  POST /analyze-direct            Run full 6-step pipeline on raw extracted_data JSON
  POST /report                    Pipeline + structured 6-section UI report
  POST /report/markdown           Same as /report but returns Markdown plain text
  GET  /full-result/{task_id}     Extraction + full pipeline result for a completed task
  GET  /report/{task_id}          6-section UI report for a completed task

All cost figures are in INR.
"""
