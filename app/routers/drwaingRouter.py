...
@router.get("/material-density/{material_name}")
async def get_material_density_info(material_name: str):
    """
    Get density information for a specific material.
    """
    try:
        from app.service.volume_calculation_service import get_material_density
        ...
        return {
            ...
            "conversions": {
                "1_cm3_weighs_grams": density * 1000,
                "1_m3_weighs_kg": density * 1000000,
                "1000_mm3_weighs_grams": density * 1000
            }
        }
        ...
        logger.error(f"Error retrieving material density: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Could not retrieve material density: {str(e)}"
        )
