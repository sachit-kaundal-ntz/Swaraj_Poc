        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # SIGNIFICANTLY INCREASE max_output_tokens
                    ...
                    logger.info(f"Attempt {attempt+1} - Raw response length: {len(response_text)} characters")

                    # Check if response appears truncated
                    if len(response_text) > 30000:  # Very long response likely complete
                        logger.info(f"Long response received ({len(response_text)} chars), likely complete")
                    elif not response_text.endswith('}') and '}' in response_text:
                        # Response might be truncated - try to find the last complete JSON object
                        last_brace = response_text.rfind('}')
                        if last_brace != -1:
                            response_text = response_text[:last_brace + 1]
                            logger.info(f"Response appeared truncated, extracted up to last complete brace")
                    ...
                    # Enhanced JSON validation with better error reporting
                    try:
                        # Try to parse as-is first
                        ...
                        logger.warning(f"Attempt {attempt+1} - Initial JSON parse failed, attempting fixes: {str(e)}")
                        
                        # Try multiple fix strategies
                        fixed_response_text = self.attempt_fix_json(response_text)
                        fixed_response_text = self.validate_and_fix_json(fixed_response_text)
                        
                        try:
                            parsed_data = json.loads(fixed_response_text)
                            response_text = fixed_response_text
                            output_tokens = self.count_tokens_accurate(response_text)
                            logger.info(f"JSON fixes successful")
                            
                        except json.JSONDecodeError as e2:
                            logger.error(f"Attempt {attempt+1} - JSON fixes failed: {str(e2)}")
                            
                            # Try partial extraction from what we have
                            partial_data = self._attempt_partial_extraction(response_text)
                            if partial_data and "note" not in partial_data:
                                logger.info(f"Partial extraction successful with {len(partial_data)} items")
                                parsed_data = {
                                    "drawing_metadata": partial_data,
                                ...
                                if attempt == max_retries - 1:
                                    return {
                                        "error": f"Invalid JSON response after all fixes: {str(e2)}",
                    ...
                    if "partial_extraction" in parsed_data:
                        parsed_data["warning"] = "Partial data extracted due to JSON parsing issues"
...
#                     # Use centralized configuration - REMOVED DUPLICATE max_output_tokens
#                     response = model.generate_content(
#                         [PROMPT, uploaded_file],
#                         generation_config={
#                             "temperature": 0.0,
#                             "max_output_tokens": self.MAX_ALLOWED_OUTPUT_TOKENS  # Using the class constant
#                         },
#                         safety_settings=safety_settings
#                     )
...
#                         "input_tokens": input_token_info,
#                         "output_tokens": output_tokens,
#                         "output_token_limit": self.MAX_ALLOWED_OUTPUT_TOKENS,  # Using the class constant
#                         "total_tokens": total_tokens,
#                         "token_counting_method": "gemini_api_with_estimation_fallback",
#                         "image_dimensions": image_dims,
#                         "response_length_chars": len(response_text)
