...
# ──────────────────────────────────────────────────────────────────────────────
# VolumeCalculationService class
# Required by volume_router.py which calls:
#   volume_service = compute_volume()               ← instantiates this class
#   volume_service.calculate_from_json_file(...)    ← method call
#   volume_service.calculate_from_extracted_data(...)
# ──────────────────────────────────────────────────────────────────────────────

class VolumeCalculationService:
    """
    Class wrapper around compute_volume() so that volume_router.py can use:
        volume_service = compute_volume()
        volume_service.calculate_from_json_file(path)
        volume_service.calculate_from_extracted_data(data)
    """
    ...
    def calculate_from_extracted_data(
        ...
        """
        Calculate volume from an extracted_data dict.
        include_bore_subtraction is handled automatically by the CSG logic.
        """
        try:
            result = compute_volume(extracted_data)

            # Build dims_used summary for the router's /summary endpoint
            dims_index = {
                d["id"]: float(d.get("value", 0) or 0)
                for d in extracted_data.get("dimensions", [])
                if d.get("value") is not None
            }

            # Material from title_block for mass display
            mat_text = (
            ...
            # Default density (alloy steel) for mass preview
            density = 7.85
            mass_kg = result["net_volume_cm3"] * density / 1000.0

            return {
                "status": "success",
                "volume_summary": {
                    "net_volume_mm3":  result["net_volume_mm3"],
                    "total_volume_cm3": result["net_volume_cm3"],
                    "method":          result["method"],
                    "feature_breakdown": result["feature_breakdown"],
                },
                "mass_calculation": {
                    "calculated_mass_kg": round(mass_kg, 4),
                    "calculated_mass_g":  round(mass_kg * 1000, 2),
                    "material":           mat_text,
                    "density_g_cm3":      density,
                    "note": "Density is default alloy-steel 7.85 g/cm³. Run full pipeline for accurate material match.",
                },
                "dimensions_used": {
                    "gear_outer_diameter_mm": dims_index.get("dim_outer_dia"),
                    "gear_face_width_mm":     dims_index.get("dim_face_width"),
                    "hub_outer_diameter_mm":  dims_index.get("dim_hub_dia"),
                    "hub_height_mm":          dims_index.get("dim_hub_height"),
                    "bore_diameter_mm":       dims_index.get("dim_bore"),
                },
            }
    ...
    def calculate_from_json_file(
        ...
        """
        Load extracted_data from a JSON file on disk, then calculate volume.
        """
        ...
        try:
            ...
            extracted_data = raw.get("extracted_data", raw)
            return self.calculate_from_extracted_data(extracted_data, include_bore_subtraction)
...
# ──────────────────────────────────────────────────────────────────────────────
# Make compute_volume() work BOTH as a plain function call AND as a class
# instantiation — so volume_router.py line:
#     volume_service = compute_volume()
# returns a VolumeCalculationService instance instead of crashing.
# ──────────────────────────────────────────────────────────────────────────────

class _VolumeCallable:
    """
    Callable that behaves as both a function and a class constructor.

    When called WITH extracted_data argument  → runs the pipeline, returns dict
    When called WITH NO arguments             → returns a VolumeCalculationService instance
    """

    def __call__(self, extracted_data: dict = None):
        if extracted_data is None:
            # volume_router.py usage: volume_service = compute_volume()
            return VolumeCalculationService()
        # pipeline_service.py usage: result = compute_volume(extracted_data)
        if extracted_data.get("assembly_order"):
            result = _csg_method(extracted_data)
        else:
            result = _fallback_dim_method(extracted_data)
...
# Replace the plain function with the smart callable
compute_volume = _VolumeCallable()