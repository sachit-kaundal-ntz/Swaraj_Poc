...
from .operation  import FEATURE_PROCESS_MAP   # feature type+role → operations
...
def match_material(text: str) -> dict:
    ...
    Lookup order: all non-default keys → default-steel (fallback).

    Returns:
        {matched_key, density, rate [INR/kg], matched_pattern}
    """
    if not text:
...
def compute_material_cost(extracted_data: dict, net_volume_cm3: float) -> dict:
    ...
    Returns:
    {
        matched_material, matched_pattern, density_g_cm3,
        rate_per_kg [INR/kg], net_volume_cm3,
        mass_kg, cost [INR], currency, source
    }
    """
    ...
    return {
        ...
        "rate_per_kg":       mat_info["rate"],           # INR/kg
...
def _op_cost(estimated_time_min: float, rate_per_hour: float) -> float:
    """Per-op cost formula: (time_min / 60) × rate_per_hr  [INR]"""
    return (estimated_time_min / 60.0) * rate_per_hour


def _get_template(gear_family: str, op_name: str) -> dict:
    """
    Look up operation template dict (INR rates from gear_rule.py).
    Falls back to spur family, then bare defaults.
    """
    ...
    for tmpl in OPERATIONS_TEMPLATES.get("spur", []):
        if tmpl["operation"].lower() == op_name.lower():
            return tmpl
    # bare fallback — ₹5040/hr (= $60 × 84)
    return {
        "operation":          op_name,
        "description":        "",
        "estimated_time_min": 15,
        "rate_per_hour":      round(60 * 84, 2),
        "machine_options":    [],
    }
...
def build_operations_with_costs(
    ...
    """
    Step 4 — Build merged operations list with per-op costs (all in INR).
    ...
    [
      {
        ...
        estimated_time_min, rate_per_hour [INR/hr], cost [INR],
    ...
    for step_no, (op_name, source) in enumerate(ordered, start=1):
        ...
        result.append({
            ...
            "rate_per_hour":      tmpl["rate_per_hour"],    # INR/hr
            "cost":               round(cost, 2),           # INR
...
def compute_full_cost(
    ...
    Returns:
    {
        ...
        operations_detail: [...],
        cost_breakdown:    {
            material_cost [INR], operations_cost [INR],
            overhead_pct, overhead_cost [INR],
            subtotal [INR], margin_pct, margin_cost [INR],
            total_cost [INR], currency
        }
    }
    """
    # Step 3 — material
